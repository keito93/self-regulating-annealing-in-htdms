...
def resolve_path(path_str: str, base_dir: Path) -> Path:
    ...
    Paths stored in metadata.json may be either absolute paths or relative paths.
    Relative paths are usually written from the project root by sample.py, but in
    some cases they may be relative to sample_dir. Try both conventions.
    """
    path = Path(path_str)
    if path.is_absolute():
        return path
